...
usage() {
  cat <<'USAGE' >&2
Usage: publish-asset-compress.sh [OPTIONS]

Tag, build, and push the asset-compress container image to Docker Hub (or any OCI registry).
...
# Release configuration for the asset-compress image.
# Fill in these defaults with your Docker Hub (or other registry) settings.
# Example values:
#   OCI_IMAGE_REPOSITORY=docker.io/youruser/asset-compress
